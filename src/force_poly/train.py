"""Training loop cho PolynomialRegressor.
...
from .dataset import build_datasets, discover_trials, split_trials
from .features import feature_dim, feature_names
from .model import PolynomialRegressor, count_parameters
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(description="Train PolynomialRegressor.")
    ...
    if not cache_dir.is_dir():
        raise SystemExit(
            f"cache_dir không tồn tại: {cache_dir}. "
            f"Chạy `python -m src.force_model.prepare` trước."
        )
    ...
    train_ds, val_ds, test_ds = build_datasets(split, feature_set=feature_set)
    logger.info(
        "Frames: train=%d, val=%d, test=%d | features=%d (%s)",
        len(train_ds), len(val_ds), len(test_ds),
        feature_dim(feature_set), feature_set,
    )
    ...
    model.fit_scaler(train_ds.feature_matrix())
    logger.info(
        "Model: degree=%d head=%s n_terms=%d params=%d",
        model.degree, model.head_kind, model.n_terms, count_parameters(model),
    )
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=n_epochs, eta_min=float(cfg["train"]["lr"]) * 0.01,
    )
