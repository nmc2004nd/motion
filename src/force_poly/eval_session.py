"""Evaluate best force_poly checkpoint on one recorded session.

Usage:
    python -m src.force_poly.eval_session \
        --config src/force_poly/config.yaml \
        --session /home/nmc/ManhCuong/motion/data/sessions/3

This does not use train/val/test split. It evaluates every cached trial that
belongs to the given session directory. Raw sessions must already have matching
cache files from ``src.force_model.prepare``.
"""
...
from .dataset import PolyFeatureDataset
from .eval import (
    _dump_linear_coefficients,
    _metrics,
    _plot_scatter,
    _plot_timeseries,
    load_yaml,
...
def _build_model(cfg: dict, feature_set: str, ckpt_path: Path, device: torch.device) -> PolynomialRegressor:
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Eval PolynomialRegressor on all trials from one session.",
    )
    ...
    parser.add_argument("--out", default="outputs/force_poly/eval_session")
    ...
    dataset = PolyFeatureDataset(cache_paths, feature_set=feature_set)

    ckpt_path = Path(args.ckpt or (Path(cfg["train"]["checkpoint_dir"]) / "best.pt"))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = _build_model(cfg, feature_set, ckpt_path, device)
    ...
    _dump_linear_coefficients(model, feature_set, out_dir / f"{prefix}_coefficients.csv")
